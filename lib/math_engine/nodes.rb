module MathEngine
  class MathEngine
    ...
    class ExponentNode < Node
      ...
      def to_tex
        if right.class == LiteralNumberNode &&
          left.class == LiteralNumberNode
          "#{left.to_tex} ^ #{right.to_tex}"
        elsif
          left.class == LiteralNumberNode &&
            right.class != LiteralNumberNode
          "#{left.to_tex} ^ {#{right.to_tex}}"
        elsif
          right.class == LiteralNumberNode &&
            left.class != LiteralNumberNode
    ...
    class FunctionCallNode < Node
      ...
      def to_tex
        if right.left.left.class == LiteralNumberNode ||
          right.left.left.class == IdentifierNode
          "\\#{left} #{right.to_tex}"
        else
          "\\#{left}#{right.to_tex}"
        end
    ...
    class ParametersNode < Node
      ...
      def parenthesis(node)
        if node.class == LiteralNumberNode ||
          ...
          (node.left.class == IdentifierNode && node.right.nil?) ||
          (node.left.class == LiteralNumberNode && node.right.nil?)
          node.to_tex
        else
          "(#{node.to_tex})"
        end
